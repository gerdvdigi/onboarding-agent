...
export function normalizeMarkdown(content: string): string {
  ...
  normalized = normalized.replace(
    /(Stage|Step|Process|Setup|Automation)([A-Z][a-z])/g,
    '$1\n\n$2',
  );
  normalized = normalized.replace(
  ...
  normalized = normalized.replace(
    /(Helpful Articles)(:\s*-\s*)([A-Za-z])/g,
    '$1:\n\n- $3',
  );
  normalized = normalized.replace(
    /(Helpful Article[s]?):([A-Za-z\[\-])/g,
    '$1: $2',
  );
  normalized = normalized.replace(
    /(Helpful Article[s]?):-(?=\S)/g,
    '$1:\n\n- ',
  );
  ...
  normalized = normalized.replace(
    /([a-zA-Z])(\d+\s+(?:days?|weeks?|hours?|minutes?|emails?))/gi,
    '$1 $2',
  );
  normalized = normalized.replace(/(over|after|in|for)(\d)/gi, '$1 $2');
  normalized = normalized.replace(
    /(Persona|Campaign|Step|Stage|Process|Pipeline)(\d)/gi,
    '$1 $2',
  );
  ...
  normalized = normalized.replace(
    /([a-z])\*\*(?=Where|Helpful|Properties|Automations|Campaign|Persona|Technical)/g,
    '$1\n\n**',
  );
