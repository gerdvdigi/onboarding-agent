...
/** Patrones que indican descripción de audiencia/lead, no objetivos. */
const AUDIENCE_FRAGMENT_PATTERNS = [
  /^we're\s+/i,
  /^we are\s+/i,
  /^a good lead\s/i,
  /^mainly\s+/i,
  /^mostly\s+/i,
  /^(founders?|owners?|ceos?|heads?)\s+(of\s+)?(marketing|sales|small|mid-sized)/i,
  /^(founders?|owners?|ceos?|heads?)$/i,
  /^decision\s+maker/i,
  /^b2b\s+/i,
];

function looksLikeAudienceFragment(text: string): boolean {
  const t = text.trim();
  if (t.length < 4 || t.length > 150) return false;
  return AUDIENCE_FRAGMENT_PATTERNS.some((p) => p.test(t));
}
...
function extractObjectives(answers: Record<string, unknown>): string[] {
  ...
  // Solo el primer segmento (antes del primer |): son los objetivos del hub.
  // Los demás son audiencia, lead criteria, canales, etc.
  const firstSegment = hubSpecific.split(/\|/)[0]?.trim() ?? '';
  if (firstSegment) {
    const parts = firstSegment
      .split(/[,;]|\s+and\s+|\n+/i)
      .map((s) => s.trim())
      .filter((s) => s.length > 3 && s.length < 200)
      .filter((s) => !looksLikeAudienceFragment(s));
...
const GeneratePlanDraftSchema = z.object({
  ...
  knowledgeContext: z
    .string()
    .describe('Technical insights retrieved from the RAG search'),
  answersCollected: z
    .record(z.string(), z.any())
    .optional()
    .describe(
      'The full set of discovery data (company_info, hubs_included, subscription_levels, overall_goals, hub_specific_details).',
    ),
});
...
export const generatePlanDraftTool = tool(
  async (input: z.infer<typeof GeneratePlanDraftSchema>): Promise<string> => {
    ...
    console.log(
      '[generate_plan_draft] Hubs:',
      answersCollected.hubs_included?.substring(0, 80),
    );
    ...
    const plan: ImplementationPlan = {
      ...
      hub_sales: activeHubs.sales,
      hub_marketing: activeHubs.marketing,
      hub_services: activeHubs.service,
    };
    ...
    return JSON.stringify({
      ...
      company: companyName,
      activeHubs: hubsList,
      subscriptionLevel: parseSubscriptionLevel(
        answersCollected.subscription_levels,
      ),
      objectives: objectives,
      timeline: plan.timeline,
      ragContext: knowledgeContext
        ? 'Available - use format from knowledge base'
        : 'Not available - use system prompt format',
      instruction:
        'Generate the Implementation Plan now using the PHASE 2 format from your system prompt. Do NOT output this message to the user.',
    });
  },
  {
