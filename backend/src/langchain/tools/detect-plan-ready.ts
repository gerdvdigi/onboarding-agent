...
const DetectPlanReadySchema = z.object({
  questionsAsked: z
    .array(z.string())
    .optional()
    .describe('List of questions already asked by the agent'),
  answersCollected: z
    .record(z.string(), z.any())
    .optional()
    .describe('Data points gathered from the user'),
...
function parseActiveHubs(hubsText: string): {
  ...
  const salesExcluded = /sales\s+(can\s+)?(come|wait|later|not\s+now)/i.test(
    text,
  );
  const marketingExcluded =
    /marketing\s+(can\s+)?(come|wait|later|not\s+now)/i.test(text);
...
function checkHubQuestionsAsked(
  ...
  console.log(
    '[detect_plan_ready] Questions text (first 500 chars):',
    allQuestionsText.slice(0, 500),
  );
  ...
  if (activeHubs.sales) {
    ...
    console.log(
      '[detect_plan_ready] Sales patterns matched:',
      salesQuestionsFound,
      salesMatches,
    );
    if (salesQuestionsFound < 3) {
      missingHubQuestions.push(
        `STEP 6A (Sales) incomplete: ${salesQuestionsFound}/3 patterns matched`,
      );
    }
  }
  ...
  if (activeHubs.service) {
    ...
    console.log(
      '[detect_plan_ready] Service patterns matched:',
      serviceQuestionsFound,
      serviceMatches,
    );
    if (serviceQuestionsFound < 3) {
      missingHubQuestions.push(
        `STEP 6B (Service) incomplete: ${serviceQuestionsFound}/3 patterns matched`,
      );
    }
  }
  ...
  if (activeHubs.marketing) {
    ...
    const marketingMatches = marketingPatterns.map((p) =>
      p.test(allQuestionsText),
    );
    const marketingQuestionsFound = marketingMatches.filter(Boolean).length;
    console.log(
      '[detect_plan_ready] Marketing patterns matched:',
      marketingQuestionsFound,
      marketingMatches,
    );
    if (marketingQuestionsFound < 4) {
      missingHubQuestions.push(
        `STEP 6C (Marketing) incomplete: ${marketingQuestionsFound}/4 patterns matched`,
      );
    }
  }
...
export const detectPlanReadyTool = tool(
  async (input: z.infer<typeof DetectPlanReadySchema>) => {
    ...
    console.log(
      '[detect_plan_ready] - First 3 questions:',
      history.slice(0, 3).map((q) => q.substring(0, 50)),
    );
    ...
    const basePillarConfidence = Math.round(
      ((REQUIRED_PILLARS.length - missingPillars.length) /
        REQUIRED_PILLARS.length) *
        100,
    );
    // Reduce confidence if hub questions are incomplete
    const confidence = hubQuestionsCheck.complete
      ? basePillarConfidence
      : Math.min(basePillarConfidence, 70);
  ...
  },
  {
    ...
    description:
      'Analyzes if the discovery phase is complete. Checks for: company_info, hubs_included, subscription_levels, overall_goals, hub_specific_details, AND verifies that Hub-specific questions (6A/6B/6C) have been asked for all selected Hubs.',
